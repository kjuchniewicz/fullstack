...
func (u *User) UpdateAUser(db *gorm.DB, uid int32) (*User, error) {
	...
	db = db.Debug().Model(&User{}).Where("uid = ?", uid).Take(&User{}).UpdateColumns(
	...
	err = db.Debug().Model(&User{}).Where("uid = ?", uid).Take(&u).Error
	if err != nil {
		return &User{}, err
	}
	return u, nil
...
func (u *User) DeleteAUser(db *gorm.DB, uid uint32) (int64, error) {
	db = db.Debug().Model(&User{}).Where("uid = ?", uid).Take(&User{}).Delete(&User{})
	if db.Error != nil {
		return 0, db.Error
	}
